/*
  # Création des tables pour la gestion financière avec dates de paiement

  1. Nouvelles tables
    - `depenses` - Table pour stocker les dépenses avec date de paiement
    - `recettes_exceptionnelles` - Table pour stocker les recettes exceptionnelles avec date de paiement
    - `ristournes` - Table pour stocker les ristournes avec date de paiement
    - `sinistres` - Table pour stocker les sinistres avec date de paiement

  2. Colonnes de dates
    - Chaque table a une date d'événement (date_depense, date_recette, etc.)
    - Chaque table a une date de paiement (date_paiement_depense, date_paiement_recette, etc.)
...
CREATE TABLE IF NOT EXISTS recettes_exceptionnelles (
  ...
  date_paiement_recette DATE DEFAULT CURRENT_DATE,
  cree_par TEXT NOT NULL,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS ristournes (
  ...
  date_paiement_ristourne DATE DEFAULT CURRENT_DATE,
  cree_par TEXT NOT NULL,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS sinistres (
  ...
  date_paiement_sinistre DATE DEFAULT CURRENT_DATE,
  cree_par TEXT NOT NULL,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE INDEX IF NOT EXISTS depenses_date_paiement_idx ON depenses (date_paiement_depense);
...
CREATE INDEX IF NOT EXISTS recettes_date_paiement_idx ON recettes_exceptionnelles (date_paiement_recette);
...
CREATE INDEX IF NOT EXISTS ristournes_date_paiement_idx ON ristournes (date_paiement_ristourne);
...
CREATE INDEX IF NOT EXISTS sinistres_date_paiement_idx ON sinistres (date_paiement_sinistre);
...
BEGIN
    ...
    RAISE NOTICE '   - Table depenses avec date_depense et date_paiement_depense';
    RAISE NOTICE '   - Table recettes_exceptionnelles avec date_recette et date_paiement_recette';
    RAISE NOTICE '   - Table ristournes avec date_ristourne et date_paiement_ristourne';
    RAISE NOTICE '   - Table sinistres avec date_sinistre et date_paiement_sinistre';
